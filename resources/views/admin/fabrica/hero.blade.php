...
@section('title', 'Hero — Fábrica de software')
@section('breadcrumb', 'Fábrica de software › Banner / Hero')
...
<div class="form-page-header">
    ...
    <div class="fph-text">
        <h2>Banner / Hero</h2>
        <p>Sección de portada de la página Fábrica de software</p>
    </div>
...
<form action="{{ route('admin.fabrica.hero.update') }}" method="POST" enctype="multipart/form-data">
@csrf
        ...
        <div class="form-card" style="margin-bottom:18px">
            ...
            <div class="form-card-body">
                <div class="field-group">
                    <label class="field-label" for="badge_text">Etiqueta de aniversario</label>
                    <span class="field-hint">Ej: "20 Aniversario"</span>
                    <input
                        type="text"
                        id="badge_text"
                        name="badge_text"
                        ...
                        placeholder="20 Aniversario"
                    >
                    <div class="field-counter"><span id="badge-count">{{ strlen($section->content('badge_text', '')) }}</span>/100</div>
                    @error('badge_text')<p style="color:#ef4444;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>

                <div class="field-group">
                    <label class="field-label" for="tag">Etiqueta superior <span style="color:#94a3b8;font-weight:400">(texto pequeño)</span></label>
                    <span class="field-hint">Aparece sobre el título principal. Ej: "Fábrica de software"</span>
                    <input
                        type="text"
                        id="tag"
                        name="tag"
                ...
                <div class="field-group">
                    ...
                    <span class="field-hint">Texto grande del hero. Puedes usar &lt;em&gt;&lt;/em&gt; para texto en cursiva azul.</span>
                    <input
                        type="text"
                        id="title"
                        name="title"
                        ...
                        maxlength="200"
                        oninput="updatePreview()"
                        placeholder="Software a la medida"
                    >
                ...
                <div class="field-group">
                    ...
                    <span class="field-hint">Párrafo descriptivo debajo del título.</span>
                    <textarea
                        id="description"
                        name="description"
                        class="field-input"
                        maxlength="800"
                        oninput="updatePreview()"
                        placeholder="Ayudamos a las empresas a crecer..."
                    >{{ old('description', $section->content('description')) }}</textarea>
                    <div class="field-counter"><span id="desc-count">{{ strlen($section->content('description', '')) }}</span>/800</div>
        ...
        <div class="preview-card">
            ...
            <div class="preview-img-wrap">
                ...
                <div class="preview-overlay">
                    <div class="preview-badge" id="prev-badge">{{ $section->content('tag') }}</div>
                    <div class="preview-title" id="prev-title">{!! strip_tags($section->content('title')) !!}</div>
...
<script>
    function updatePreview() {
        ...
        document.getElementById('prev-title').textContent = title.replace(/<\/?[^>]+(>|$)/g, ""); // strip HTML tags
        document.getElementById('prev-desc').textContent  = desc;

        document.getElementById('badge-count').textContent = badge.length;
        document.getElementById('tag-count').textContent = tag.length;
